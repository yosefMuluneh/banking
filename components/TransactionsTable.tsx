        ...
        <TableBody>
            {
                transactions.map((tra: Transaction)=>{
                    ...
                    const isCredit = tra.type === 'credit'

                    return (
                        <TableRow className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !border-b-DEFAULT `} key={tra.id}>
                            <TableCell className='max-w-[250px] pl-2 pr-10'>
                                <div className='fle item-center gap-3'>
                                    <h1 className='text-14 truncate font-semibold tect-[#344054]'>
                                        { removeSpecialCharacters(tra.name)}
                                    </h1>
                                </div>
                            </TableCell>
